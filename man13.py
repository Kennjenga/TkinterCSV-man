...
from dbconn2 import check_GL_contra_codes, delete_GL_contra_codes, get_account_by_gl_contra_code, new_GL_contra_codes, update_GL_contra_codes
...
def GL_code():
    def check_submit():
        ...
        account_info = check_GL_contra_codes(
            gl_contra_code, message_var, Account_var)
        return account_info
    ...
    def update_submit():
        ...
        if not account or (not account.isdigit() and account != '0'):
            message_var.set("Enter the correct format (numeric value)")
            return None  # Return None to indicate an error

        account_info = update_GL_contra_codes(
            gl_contra_code, message_var, account)
        return account_info
    ...
    def new_submit():
        ...
        account_info = new_GL_contra_codes(
            gl_contra_code, message_var, Account_var, account)
        return account_info

    def delete_submit():
        gl_contra_code = GL_code_var.get()

        delete_GL_contra_codes(gl_contra_code, message_var)
        GL_code_var.set("")
        Account_var.set("0")
    ...
    button_frame = tk.Frame(GLframe)
    button_frame.pack(padx=20, pady=30)

    Check_GLcode = tk.Button(
        GLframe, text="Check GL_code", command=check_submit)
    Update_GLcode = tk.Button(
        GLframe, text="Update GL_code", command=update_submit)
    New_GLcode = tk.Button(GLframe, text="New GL_code", command=new_submit)
    Delete_GLcode = tk.Button(
        GLframe, text="Delete GL_code", command=delete_submit)

    # Approach 1: Using `pack` with `expand=True` (Grid-like behavior)
    Check_GLcode.pack(side="left", in_=button_frame, padx=10)
    Update_GLcode.pack(side="left", in_=button_frame, padx=10)
    New_GLcode.pack(side="left", in_=button_frame, padx=10)
    Delete_GLcode.pack(side="left", in_=button_frame, padx=10)
