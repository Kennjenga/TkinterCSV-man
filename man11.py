...
def GL_code():
    ...
    gl_contra_label = tk.Label(GLframe, text="GL_Contra_codes", bg="yellow")
    gl_contra_label.pack()
    GLframe.pack(expand=True, fill="both", padx=10, pady=10)
    GLframe.configure(width=400, height=400)

    GL_code_var = tk.StringVar()
    GL_code_label = tk.Label(GLframe, text="Enter GL Contra Code")
    GL_code_label.pack()
    GL_code_label.configure(width=100)
    GL_code_Entry = tk.Entry(GLframe, textvariable=GL_code_var)
    GL_code_Entry.pack()
    GL_code_Entry.configure(width=100)
    # Optional: Configure frame size (might be controlled by pack already)
    # GLframe.configure(width=400, height=400)
    Account_var = tk.StringVar()
    Account_label = tk.Label(GLframe, text="Enter GL Contra Code")
    Account_label.pack()
    Account_label.configure(width=100)
    Account_Entry = tk.Entry(GLframe, textvariable=Account_var)
    Account_Entry.pack()
    Account_Entry.configure(width=100)
